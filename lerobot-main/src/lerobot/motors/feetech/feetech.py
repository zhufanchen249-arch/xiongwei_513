...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def set_half_turn_homings(
        ...
        # Toggle torque to force the motor to reload EPROM (STS3215 caches homing offset
        # after power-up and won't see the zeroed value until torque is cycled).
        self.enable_torque(motor_names)
        self.disable_torque(motor_names)
        # Use sequential reads instead of sync_read — GroupSyncRead is unreliable on STS3215.
        actual_positions: dict[NameOrID, int] = {}
        for motor in motor_names:
            actual_positions[motor] = self.read("Present_Position", motor, normalize=False)
    ...
    def _seq_read_positions(self, motor_names: list[str]) -> dict[str, int]:
        """Sequential position read — sidesteps GroupSyncRead issues with STS3215."""
    ...
    def record_ranges_of_motion(
        ...
        start_positions = self._seq_read_positions(motor_names)
        mins = start_positions.copy()
        maxes = start_positions.copy()

        user_pressed_enter = False
        while not user_pressed_enter:
            positions = self._seq_read_positions(motor_names)
